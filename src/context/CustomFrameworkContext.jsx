...
export function useCustomFramework() {
  ...
  if (!context) {
    throw new Error('useCustomFramework must be used within CustomFrameworkProvider')
  }
  return context
}
...
export function CustomFrameworkProvider({ children }) {
  ...
  const addControls = (controls, frameworkInfo) => {
    ...
    setSelectedControls(prev => {
      const existing = new Set(prev.map(c => `${c.sourceFramework.id}-${c.id}`))
      const filtered = newControls.filter(c => !existing.has(`${c.sourceFramework.id}-${c.id}`))
  ...
  const removeControl = (controlId, frameworkId) => {
    setSelectedControls(prev =>
      prev.filter(c => !(c.id === controlId && c.sourceFramework.id === frameworkId))
    )
  }
